...
- **A1a — hozir:** `.dockerignore`; local CI required checks; ~~health/readiness contract~~ **(bajarildi 2026-08-15: `/healthz` + `/readyz`, 9 test)**; static build; secret/dependency scan; ~~Telegram outbox claim/lease~~ **(bajarildi 2026-08-15: atomik claim + lease expiry, 8 test, Control Center `in_flight` ko'rsatkichi)** va local e2e; reproducible local backup/restore.
