...
*Model: kontentning YADROSI — Azurbekning kursi (u yillardir o'tayotgan dastur). Platforma-kontent uni o'raydi: har darsga video + konspekt + lug'at + quiz + vazifa; ustiga mock imtihonlar va oqim-kontenti. Eng uzun ustun endi matn yozish emas — **video yozib olish (Azurbek vaqti)**; qolganini AI-quvur ko'taradi.*

---

## 1. Kurs strukturasi — manba: Azurbekning mavjud dasturi

**P0.3 vazifasi:** Azurbek joriy o'quv dasturini shu shablonga tushiradi (Claude bilan birga, 1–2 soat suhbat yetadi):
...
|---|---|---|
| 1. **Video dars** (10–20 daq, yozib olingan) | Azurbek | jonli dars o'tilgach (A0 tugmasi) |
...
| 4. Lug'at (8–12 so'z: tr/uz/misol/🔗kognat) → **SRS'ga avto** | AI-qoralama → tahrir | video bilan |
...
| 7. AI mashq CTA ("Azure bilan shu mavzuni mashq qil" → `?lesson=` deep-link) | avtomatik | doim |
...
## 3. Video yozish jadvali (P0.4'da kelishiladi — REAL bo'lsin)

Gipoteza-reja (Azurbek tasdiqlaydi/to'g'irlaydi):

| Davr | Norma | Jami |
|---|---|---|
| P1 (16 iyul – 5 avg) | haftasiga 4–5 video | ~12–15 dars |
| P2 (6–24 avg) | haftasiga 4–5 video | ~12–15 dars (jami ~25–30) |
| P3 (25 avg – 7 sen) | haftasiga 2–3 (guruh boshlangan — jonli yuk) | jami ~32–36 |
| P4 | 13-sen muzlatish | — |
...
**Sifat standarti (bir marta, P1 boshida):** yozish sozlamasi (yorug'lik/mikrofon/ekran-yozuv sxemasi), intro/outro shablon, "bir video = bir mavzu ≤20 daq" qoidasi.

## 4. Milliy sertifikat mock imtihonlari (IH-3 — S1'ning yuragi)

**P0.6:** UZBMB turk tili spetsifikatsiyasi/demo variantini olish (rasmiy sahifa + topshirgan o'quvchilar tajribasi — Azurbekning o'quvchilarida bor!). Format ma'lum qadar: ko'p darajali (A1–C2), yozma kun = tinglash+o'qish+yozish, og'zaki alohida.

| Mock | Tarkib | Tayyor bo'lish |
|---|---|---|
| **Mock №1 (yozma)** — bepul qatlamda, lead-magnit | Tinglash (2 audio, replay-limit) + O'qish (2 matn, aralash task turlari) + Yozish (2 prompt, so'z chegarasi) | P2-h3 |
| **Mock og'zaki** | 3 prompt (audio yozish) + ustoz baholash oqimi (grading bor) + conversation_partner bilan tayyorlanish | P3-h1 |
| **Mock №2 (yozma, to'liq)** | №1 formatida yangi variantlar — kurs obunachilariga | P3-h2 |

Audio manba: TTS (turkcha ovozlar) + kerak joyda Azurbek ovozi. Har mock natijasi: ball + bo'lim kesimi + zaif mavzular AI xotirasiga (dvigatelda per-esse feedback bor).
...
AI-qoralama → tahrir; haftasiga 2 post P3'dan; har biri kanalga parchalanadi.

## 8. Marketing mikro-kontent

- Ko'prik kartalari (30) — asosiy oziqa
- **Bitiruvchi hikoyalari** (P0.7'dan): "X ball oldim, magistraturaga kirdim" — eng kuchli format
...
1. Lesson'ga **lug'at strukturasi** (SRS oziqasi, A4c) — `LessonVocabulary` yoki structured blok; P1-h3 hal
2. **Preview bayroq** (bepul namunaviy dars, A10)
3. **Seed quvuri:** `load_course_content` management command (qoralamalarni bir urishda kiritish; backoffice keyin tahrir)
4. Video hosting: MVP = YouTube unlisted embed (Lesson.video_url bor; 0 xarajat, tez) — kamchiligi: yuklab olish himoyasi zaif. Alternativ: Spaces'da mp4 + oddiy pleer. **P0 qarori.** Tavsiya: launch = YouTube unlisted, post-launch himoyalangan streaming.
5. Audio (mock tinglash): mp3 Spaces'da, `<audio>` pleer (exam dvigatelda audio-limit bor)

---

*Qisqartirish tartibi (R1 ishga tushsa): blog 12→6 → ko'priklar 30→15 → mock №2 keyinga → video norma pasayadi lekin guruhdan 2 hafta oldinda yurish saqlanadi. Dars sifati va mock №1 qurbon qilinmaydi.*
