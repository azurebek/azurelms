...
**2026-09-11 — operatsion sozlamalar yuzasi:** `/backoffice/control/runtime-settings/` — owner Telegram yetkazish tezligi/qayta urinish siyosatini (`bot.BotRuntimeSettings`) va Control Center chiroq chegaralarini (`core.OperationalSettings`) **deploy'siz** o'zgartiradi (owner qarori: qotirib qo'yilgan qiymat yo'q). A2 mutation shartlari bajarilgan: majburiy sabab, majburiy tasdiq, `SystemAuditEvent` va no-op yo'l. Ikki model Django admin'da **faqat o'qish uchun** — admin orqali tahrirlash o'zgarishni izsiz qoldirardi, ya'ni `AISettingsAdmin` dagi nuqsonni takrorlardi; mavjud AI yuzalarini retrofit qilish ochiq A2 qarzi. Qiymat o'qishda xavfsiz oraliqqa qisiladi va sozlama qatori bo'lmasa kod defaulti ishlaydi — sozlama jadvali sababli worker ham, `/readyz` ham yiqilmaydi.
