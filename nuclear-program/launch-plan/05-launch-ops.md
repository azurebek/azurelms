...
- DB atomic claim/lease va lease expiry **qurilgan** (2026-08-15, A1a): parallel workerlar bir qatorni ikki marta olmaydi va o'lgan worker qatori muzlab qolmaydi. Kafolat at-least-once bo'lgani uchun notification matni duplicate kelsa ham zarar qilmaydigan bo'lishi kerak.
- **2026-09-11 — exponential retry/backoff va terminal dead-letter `IMPLEMENTED/TESTED`:** siyosat `bot/retry_policy.py` da canonical va DM navbati (`bot/outbox.py`) bilan Classbook guruh navbati (`classbook/delivery.py`) **bittasini** ishlatadi. Uch yo'l ataylab farq qiladi: `429 Flood control` urinishni **sarflamaydi** va qatorni Telegram aytgan vaqtga suradi (aks holda 50 o'quvchiga natija DM ketayotganda xabar uch siklda `failed` bo'lib, o'quvchi natijasini hech qachon ko'rmasdi — sabab esa faqat bizning tezligimiz bo'lgan); foydalanuvchi botni bloklagan yoki bot guruhdan chiqarilgan bo'lsa qator **darhol** dead-letter (qayta urinish hech qachon yordam bermaydi va rate budjetini bekorga yeydi); qolgan nosozliklar `30s → 1m → 2m → 4m` backoff bilan, jitter bilan, 5 urinishgacha. Sikl **ichida** ham oraliq bor (~20 xabar/sekund): ilgari 25 ta `send_message` orasiz otilardi va bu 429 ning bevosita sababi edi. `next_attempt_at` va `failure_kind` maydonlari ikkala navbatda; Control Center `dead_permanent` va `waiting_backoff` ni alohida ko'rsatadi — "hech qachon tuzalmaydi" bilan "o'zi tuzaladi" bir xil son ostida turmasin. Ikkinchi replica uchun qolgani faqat owner qarori.
...
Capability registry DB, cache/channels, broker, Celery worker/beat, Telegram webhook/outbox, **Telegram dispatcher** (kiruvchi update yo'li: tiriklik + javob vaqti + xato ulushi, 2026-09-12), public/private media, AI provider, RAG, memory, backup va emailni ro'yxatlaydi. Har capability: criticality, owner, health probe, runbook, feature flag/kill switch, dependency va so'nggi heartbeat.
...
**2026-09-11/12 — operatsion sozlamalar yuzasi:** `/backoffice/control/runtime-settings/` — owner Telegram yetkazish tezligi/qayta urinish siyosatini va bot o'lchov kadensini (`bot.BotRuntimeSettings`), eslatma vaqtlari va jim soatlarni (`users.ReminderSettings`, T2) hamda Control Center chiroq chegaralarini — navbat yoshi va bot dispatcher'ining tiriklik/javob vaqti/xato foizi (`core.OperationalSettings`, T4) — **deploy'siz** o'zgartiradi (owner qarori: qotirib qo'yilgan qiymat yo'q). A2 mutation shartlari bajarilgan: majburiy sabab, majburiy tasdiq, `SystemAuditEvent` va no-op yo'l. Uch model Django admin'da **faqat o'qish uchun** — admin orqali tahrirlash o'zgarishni izsiz qoldirardi, ya'ni `AISettingsAdmin` dagi nuqsonni takrorlardi; mavjud AI yuzalarini retrofit qilish ochiq A2 qarzi. Qiymat o'qishda xavfsiz oraliqqa qisiladi va sozlama qatori bo'lmasa kod defaulti ishlaydi — sozlama jadvali sababli worker ham, `/readyz` ham yiqilmaydi.
...
**Holat (2026-08-19):** yuqoridagi ro'yxatga `ai.circuit.reset` (cooldown tozalash) va `release.decision` (owner release qarori) qo'shildi — ya'ni **release/rollback endi auditlanadi**, quyidagi 2026-08-16 yozuvidagi "keyinroq keladi" bandi yopildi. Kodda yozilayotgan amallar to'liq ro'yxati: `ai.kill_switch.enable/disable`, `ai.circuit.reset`, `brand.update`, `landing.update`, `lesson.release`, `assignment.review`, `receipt.verify`, `enrollment.transfer`, `enrollment.promote`, `private_media.denied`, `release.decision`. **2026-09-11/12 da qo'shilgani:** `settings.bot_delivery.update`, `settings.operational_thresholds.update`, `settings.reminders.update`, `settings.dispatcher_thresholds.update` (T0/T2/T4 operatsion sozlamalari) va `telegram.dead_letter.replay` (T6 — outbox replay). **Minimal ro'yxatdan hamon qolgani:** broadcast, per-user block va AI bonus.

**Holat (2026-08-16):** kill switch, brend, landing, lesson release, grade/review, receipt qarori (tasdiq va rad etilgan urinish), enrollment transfer/promotion va private-media denial ledgerda. Private-media denial ataylab cheklangan: faqat autentifikatsiyadan o’tgan aktor va 15 daqiqalik takrorlanish oynasi — ledger append-only, ya’ni skaner uni bosib keta olmasligi kerak. ~~Outbox replay auditlanmagan, chunki bunday amal hali mavjud emas~~ — **2026-09-12 da yopildi (T6):** `/backoffice/control/dead-letter/` terminal qatorni owner sababi bilan navbatga qaytaradi va `telegram.dead_letter.replay` ledgerga yoziladi. Idempotentlik shartli `UPDATE` da (`WHERE status='failed'`), ya'ni ikki marta bosish xabarni ikki marta yubormaydi; `permanent` turdagi qator uchun alohida tasdiq talab qilinadi, chunki bloklagan foydalanuvchiga qayta yuborish faqat rate budjetini yeydi. Amal ikkala navbatni ham qamraydi (DM va Classbook guruh).
...
- Backup: localda SQLite/media recoverability smoke. **2026-09-10:** PostgreSQL yo'li ham canonical servisda (`core/pg_backup.py` — `pg_dump -Fc`, so'ng `pg_restore --list` bilan tekshiruv; yarim yozilgan dump o'chiriladi). `manage.py backup_db` kengaytmani backendga qarab tanlaydi va Control Center backup probe'i endi `.dump` fayllarni ham ko'radi — ilgari u faqat `*.sqlite3` qidirib, productionda har kuni zaxira olinsa ham qizil turardi. **2026-09-11:** media zaxirasi ham canonical servisda (`core/media_backup.py` — `manage.py backup_media` public/private ildizlarni bitta tekshirilgan `.tar.gz` ga yozadi, arxiv ichida `public/`/`private/` ataylab ajratilgan, `restore_media --into` bo'sh papkaga mashq qiladi). Control Center'da alohida `media_backup` capability'si — baza zaxirasi yashil turib media zaxirasi yo'qligi bitta chiroq ostida ko'rinmay ketmasin. Shu ish paytida `deploy/docker-compose.prod.yml` dagi haqiqiy nuqson topildi: `backups/` host papkasi app konteynerlariga mount qilinmagan edi, ya'ni runbookdagi cron har kuni ishlab turgandek ko'rinib, faylni ephemeral konteyner ichiga yozib yo'qotardi. **Offsite nusxa va managed DB snapshot + encrypted export hamon ochiq** — zaxiralar o'sha EC2 diskida qoladi.
- Restore drill: yangi isolated targetga restore → migration/schema check → owner smoke → evidence timestamp. *(2026-08-21: `manage.py restore_db --input <zaxira> --into <yo'l>` — joriy bazaga tegmaydi, tiklangan nusxaning butunligi, jadval soni va migratsiya sathini chiqaradi. `--into` joriy bazani ko'rsatsa rad etiladi.)* **2026-09-10:** PostgreSQL'da `--into` alohida **baza** nomi; drill uni yaratib, tiklab, sxema hisobotini beradi. Joriy baza ustidan tiklash `restore_db` orqali **ataylab rad etiladi** — sinalmagan destruktiv yo'l falokat kunida birinchi marta yugurtiriladigan kod bo'lardi; qo'lda buyruq `deploy/README.md` §6 da. Drill'ning **o'zi** real serverda hali yugurtirilmagan — evidence ochiq.
