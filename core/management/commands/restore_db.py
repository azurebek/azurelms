...
from core.backup_service import BackupError, describe_backup, is_postgres, restore_backup
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "Drill: alohida joyga tiklaydi, joriy bazaga tegmaydi. "
                "SQLite'da fayl yo'li, PostgreSQL'da yangi baza nomi."
            ),
        )
        parser.add_argument(
            "--yes",
    ...
    def handle(self, *args, **options):
        ...
        # PostgreSQL'da joriy baza ustidan tiklash umuman qo'llab-quvvatlanmaydi
        # (`core/backup_service.py` da sabab yozilgan) — `--yes` ham yordam
        # bermaydi, servis o'zi aniq xabar bilan rad etadi.
        if not drill_target and not options["yes"] and not is_postgres():
            raise CommandError(
